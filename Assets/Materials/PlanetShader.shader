Shader "PrometheusProject/PlanetShader"
{
    Properties
    {
        ...
        _SurroundingAtmosphereColour ("SurroundingAtmosphereColour", Color) = (1,1,1,1)
        _SurroundingAtmosphereWidth ("SurroundingAtmosphereWidth", Range(1,2)) = 1.5
        _SurroundingAtmosphereArea ("SurroundingAtmosphereArea", Range(0,2)) = 0.1
    ...
    SubShader
    {
        ...
        fixed4 _SurroundingAtmosphereColour;
        fixed _SurroundingAtmosphereWidth;
        fixed _SurroundingAtmosphereArea;

        struct SurroundingAtmosphereOutput
        {
            fixed3 Albedo;
            fixed3 Normal;
            fixed3 Emission;
        ...
        {
            v.vertex.xyz = v.vertex.xyz * _SurroundingAtmosphereWidth;
        }

        void surf(Input IN, inout SurroundingAtmosphereOutput o)
        {
            o.Albedo = _SurroundingAtmosphereColour.rgb;
            o.Alpha = _SurroundingAtmosphereColour.a;
        }

        inline fixed4 LightingSurroundingAtmosphere(SurroundingAtmosphereOutput s, fixed3 lightDir, fixed3 viewDir, fixed tAtten)
        {
            fixed4 result;
            fixed2 lighting;
            lighting.x = saturate(dot(s.Normal, lightDir) + _SurroundingAtmosphereArea);
            result.rgb = lighting.x * s.Albedo;
            result.a = s.Alpha * lighting.x;
            return result;
        }
        ENDCG
